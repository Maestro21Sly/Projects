...
public class    PrefixComparator implements Comparator<Term> {
    ...
    public int compare(Term v, Term w) {
        ...
        int maxLengthForComparison = Math.min(Math.min(v.getWord().length(), w.getWord().length()),myPrefixSize);
        for (int j=0; j< maxLengthForComparison;j++){
            if(v.getWord().charAt(j) != w.getWord().charAt(j)){
                return v.getWord().charAt(j)- w.getWord().charAt(j);
            }
        }
        if (maxLengthForComparison < myPrefixSize){
