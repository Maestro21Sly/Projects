...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		if (first == -1|| last == -1) {               // prefix not found
			return new ArrayList<>();
		}
		PriorityQueue<Term> Pqueue = new PriorityQueue<>(k, Comparator.comparing(Term::getWeight));
		for (int x= first; x <= last;x++){
			if (Pqueue.size()< k || Pqueue.peek().getWeight()<myTerms[x].getWeight()){
				if (Pqueue.size()==k){
					Pqueue.poll();
				}
				Pqueue.add(myTerms[x]);
			}
		}
		// write code here for assignment
		List<Term> matchList = new ArrayList<>(Pqueue);
		Collections.sort(matchList, Comparator.comparing(Term::getWeight).reversed());
		return matchList;
	}
