...
//Setting Threshold values, solid black values are very high
//raised values so in-between black and whie still counts as white
//~350 not high enough to be black
...
// Declares variables for tracking time and whether the task was completed early
...
void setup() {
  ...
  // Set pin modes for RGB LED, sets RBG LED pins as output
  pinMode(redpin, OUTPUT);
  pinMode(greenpin, OUTPUT);
  pinMode(bluepin, OUTPUT);

  // Start with the LED off (white), all 3 colors set at max
  set_RGB(255, 255, 255);
  ...
  if (tcs.begin()) {
    ...
    while (1) //while(1) is always true so it creates a loop that will continue forever if sensor not found
      ;  // halt!
  }
  for (int i = 0; i < 5; i++) {
    teamScores[i] = -1; //initlizes team scores to -1 if we don't have score
  }
}

void loop() {
  int currentState = readQTI();
  performAction(currentState); //continously reads input from the line sensors and performs acions based on the current state
}

// Reading QTI input
int readQTI() { //readQTI() reads input from the line sensors 
                //and convert if into a binary value which represents the current state of sensors
...
long rcTime(int pin) { //used for reading the analog values from the line sensors
  pinMode(pin, OUTPUT);
  ...
  long time = micros();
  while (digitalRead(pin)) //enters a loop that continues as long as pin reads high
                          // which maens capacitor has not dischareged completely
    ;
  time = micros() - time; //time elaspsed since start of function
  return time;
}
...
void performAction(int currentState) {
  switch (currentState) {
    case 7: //white, white, white
      searchForTrack();
      break;
    case 0: //black, black, black
      stop();
      ...
      servoRight.detach();
      break;
    case 6: //white, white, black, 0 is black, 1 is white
      turnRight();   
      break;
    case 5: //white, black, white
      moveOnLine(); //(2)^2 * 1 + (2)^1 *0 + 1 = 5
      break; 
    case 4: //white, black, black
      turnRight();
      break;
    case 3: //black, white, white
      turnLeft();
      break;
    case 1: //black, black, white
      turnLeft();
      break;
    case 2:
      break;
...
int getTeamID(char c) {
  int s = (int)c - 62; // a starts from 65
  return s / 3;
}

int getTeamScore(char c) {
  int s = (int)c - 62;
  return s % 3; //mod 3, everyone has 3 characters
}
