...
bool enteredTrack = false; // Flag to indicate whether the robot has entered the circular track
...
void setup() {
  ...
  set_RGB(255, 255, 255);//turn LED off
  if (tcs.begin()) {
...
int getValue(int sensorValue) {
  ...
  return -1; // default return value to avoid compiler warning
}
...
void performAction(int finalValue) {
  switch (finalValue) {
    ...
    case 0:
      ...
      servoLeft.detach();
      servoRight.detach();
      enteredTrack = true; // Setting the flag to indicate that the robot has entered the circular track

      // Now, the robot has entered the track, and we can perform color sensing
      if (tcs.begin()) {
        Serial.println("Found sensor");
      } else {
        Serial.println("No TCS34725 found ... check your connections");
...
void changeLEDColortoGreen() {
  ...
  // Our ring is green
  set_RGB(0, 255, 0);  // For a black ring, set the color to white 
}
...
int senseObjectColor() {
  if (enteredTrack) {
    ...
    } else if (green >= 80 && 100 < red && red <= 150) {
